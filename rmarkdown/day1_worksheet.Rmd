...
So far, we haven't really made much use of the tidyverse, but the some of the most useful functions are coming up. They are often termed 'verbs' because the function name corresponds more or less to what it's _doing_ to the object. Here, we focus on verbs that [transform](https://rstudio-education.github.io/tidyverse-cookbook/transform-tables.html) data: things like filtering, selecting columns and renaming variables. You will notice that we are covering **transform** before **tidy**. The precise ordering of your own workflow might differ, but one way or another, these transform functions will be super useful early on in own work.
...
- `group_by()` to perform operations on groups
- `summarise()` create summary data frames

Along the way, we will also cover something called a pipe (coded as `%>%`). The pipe will help us link all these helpful verb functions together, ensuring we write code which is easily written, read and understood!
...
You will notice that `summarise()` create a new data frame at whatever level we have specified in `group_by()`. This is subtly (but considerably) different from `mutate()` which adds a new variable to your existing data frame. Sometimes, using `mutate()` after `group_by()` does actually make sense, but what do you think would happen? Try running the above code using `mutate()` and observe what happens.
...
So far, we have been using `n()` to count the number of rows in each unique group. But, there are plenty of other things you can do. For instance, you can also calculate the mean counts
